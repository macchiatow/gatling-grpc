...
case class GrpcActionBuilder[ReqT <: GenM, ResT <: GenM](requestName: Expression[String],
  ...
  def blockingUnaryCall(methodDescriptor: MethodDescriptor[ReqT, ResT],
                        request: Expression[GenM]): GrpcActionBuilder[ReqT, ResT] = copy(methodDescriptor = methodDescriptor, request = request)
  ...
  override def build(ctx: ScenarioContext, next: Action): Action = {
    ...
    val grpcRequestAttributes = GrpcRequestAttributes(requestName, request, methodDescriptor, channel)
